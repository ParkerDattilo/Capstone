...
def calibrate_focus(known_wavelength, known_obj_distance, resolution=5, w_range=100):
    ...
    take images. Total number of images is w_range/resolution +1, so the larger this ratio the longer calibration will take
    '''
    ...
    position = float(bestImage.split('_')[1])
    ...
    done=subprocess.run(["ssh",  "pi@192.168.5.1", "python3", f"Desktop/translation_stage/calibrate.py", known_wavelength, known_obj_distance],stdout=subprocess.DEVNULL)
...
def main():
    try:
        ...
        print("Usage: ./calibrate.py known_wavelength(nm) object_distance (mm) resolution (nm) wavelength_range (nm)")
        sys.exit(0)
